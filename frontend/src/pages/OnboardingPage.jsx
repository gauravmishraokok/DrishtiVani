...
import { User, Globe, Check, Mic, Loader2, BookOpen, GraduationCap, Volume2 } from 'lucide-react';
...
const OnboardingPage = () => {
    ...
    if (step === 1) {
      ...
      if (matches.length > 0) {
        setFormData(prev => {
          ...
          if (added.length > 0) speakText(`Added ${added.join(' and ')}. Any other subjects?`);
  ...
  }, [step, isSpeaking, speakText, formData.subjects]);

  useGlobalVoice(handleTranscriptResult, !needsInteraction);
  ...
  useEffect(() => {
    if (needsInteraction || hasSpokenRef.current[step]) return;
    if (step === 2) speakText(`Nice to meet you, ${formData.name}. Which class are you in?`);
    else if (step === 3) speakText('Great. Which language should I teach in? English or Hindi?');
    else if (step === 4) speakText('Which subjects do you want to study? You can say Science, Mathematics, or Social Science. Say Next when you are done.');
    else if (step === 5) {
  ...
  }, [step, formData, speakText, needsInteraction]);
  ...
  const handleFinish = async () => {
    ...
    try {
      let finalStudent;
      const payload = { ...formData, class_num: parseInt(formData.class_num), language: formData.language === 'English' ? 'en' : 'hi' };
      try {
      ...
      localStorage.setItem('studentLanguage', finalStudent.language || 'English');
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-6 pt-24">
        <div className="w-full max-w-2xl bg-white border border-[#E8D5C4] rounded-[2.5rem] p-10 shadow-2xl">
          <AnimatePresence mode="wait">
            {needsInteraction ? (
              <motion.div key="activate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-10 space-y-10">
                <div className="flex justify-center flex-col items-center gap-6">
                  <div className="w-24 h-24 bg-[#F97316] rounded-full flex items-center justify-center text-white shadow-xl">
                    <Volume2 size={48} />
                  </div>
                  <h1 className="text-4xl font-bold font-display">Ready to Learn?</h1>
                  <p className="text-[#6B7280] text-lg max-w-sm">Tap the button below to activate your personal voice assistant.</p>
                </div>
                <button onClick={activateAudio} className="btn-primary w-full py-8 text-2xl justify-center shadow-2xl transition-all hover:scale-105 active:scale-95">
                  Start DrishtiVani
                </button>
              </motion.div>
            ) : (
              <motion.div key="steps-container" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {step === 1 && (
